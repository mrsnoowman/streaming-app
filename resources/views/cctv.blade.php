...
<head>
    ...
    <title>CCTV Monitoring - Toll Road</title>
    ...
    <style>
        ...
        * {
        ...
        .video-card {
            ...
            backdrop-filter: blur(20px);
            transition: var(--transition-slow), background 0.3s ease, border-color 0.3s ease, box-shadow 0.3s ease;
            position: relative;
            overflow: hidden;
        }
    ...
    <div class="container">
        ...
        <div class="header">
            <div class="logo-section">
                ...
                <div>
                    <div class="title">CCTV Monitoring</div>
                    <div class="subtitle">Toll Road Surveillance</div>
                </div>
            </div>
            ...
            @forelse($cctvs as $cctv)
                <div class="video-card">
                    <div class="video-header">
                        <div class="location-name">{{ $cctv->lokasi }}</div>
                    </div>
                    <div class="video-container" id="container-cctv-{{ $cctv->id }}">
                        @if($cctv->status && $cctv->http_link)
                            <video class="video-stream" id="cctv-{{ $cctv->id }}" autoplay muted playsinline preload="metadata" data-src="{{ $cctv->http_link }}">
                                <source src="" type="application/x-mpegURL">
                            </video>
                            <button class="fullscreen-btn" onclick="toggleFullscreen('container-cctv-{{ $cctv->id }}')" title="Fullscreen">
                                <i class="fas fa-expand"></i>
                            </button>
                        @else
                            <div class="video-placeholder">
                                <i class="fas {{ $cctv->status ? 'fa-video' : 'fa-video-slash' }}" style="font-size: 32px; color: #64748b;"></i>
                                <div>{{ $cctv->status ? 'Loading...' : 'Camera Offline' }}</div>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        @if($cctvs->hasPages())
            <div class="pagination-container">
                @if($cctvs->onFirstPage())
                    <span class="pagination-btn disabled">«</span>
                @else
                    <a href="{{ $cctvs->previousPageUrl() }}" class="pagination-btn">«</a>
                @endif
                
                @foreach($cctvs->getUrlRange(1, $cctvs->lastPage()) as $page => $url)
                    @if($page == $cctvs->currentPage())
                ...
                @if($cctvs->hasMorePages())
                    <a href="{{ $cctvs->nextPageUrl() }}" class="pagination-btn">»</a>
                @else
                    <span class="pagination-btn disabled">»</span>
                @endif
                
                <div class="pagination-info">
                    Showing {{ $cctvs->firstItem() }} to {{ $cctvs->lastItem() }} of {{ $cctvs->total() }} CCTV
                </div>
            </div>
        @endif
    </div>
